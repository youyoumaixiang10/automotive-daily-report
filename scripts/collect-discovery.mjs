...
const html = await collectBrowserHtml('https://www.autohome.com.cn/news//');
const candidates = parseAutohomeNews(html);
const dongchedi = await collectDongchediNews();
...
writeJson(outputFile, {
  ...
  sourceResults: [{ sourceId: 'autohome-news', url: 'https://www.autohome.com.cn/news//', status: candidates.length ? 'ok' : 'empty', count: recentCandidates.filter(item => item.sourceId === 'autohome-news').length }, ...dongchedi.sourceResults],
